      ...
      if (orderForm.payment_type === "CashOnDelivery") {
        ...
        if (codResponse.data.codSuccess) {
          ...
          setItems((prevItems) =>
            prevItems.map((i) =>
              i.id === currentItem.id
                ? { ...i, quantity: i.quantity - orderForm.quantity }
                : i
            )
          );
          ...
          setCurrentItem((prev) => ({
            ...prev,
            quantity: prev.quantity - orderForm.quantity,
          }));
  ...
  const fetchOrdersByPhone = async () => {
    try {
      const res = await axios.get(
        `http://localhost:8080/api/orders/by-phone/${phone}`
      );
      setOrders(res.data);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col">
              ...
              orders.map((order) => (
                ...
                >
                  ...
                  <div className="flex items-start gap-4">
                    {order.item ? (
                      <img
                        src={`http://localhost:8080/${order.item.imagePath.replace(
                          /\\/g,
                          "/"
                        )}`}
                ...
                >
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    {cartItem.item?.imagePath ? (
                      <img
                        src={`http://localhost:8080/${cartItem.item.imagePath.replace(
                          /\\/g,
                          "/"
                        )}`}
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex justify-center items-center z-50">
          <div className="bg-white rounded-xl p-6 w-96">
            <h2 className="text-xl font-bold mb-4">
              Order Details for {currentItem.itemName}
            </h2>
            ...
            <input
              ...
              onChange={(e) =>
                setOrderForm({ ...orderForm, contact_number: e.target.value })
              }
            />
      ...
      <footer className="mt-auto py-6 text-gray-500 text-center">
        <p>
          © {new Date().getFullYear()} N-Tech Hardware. All rights reserved.
        </p>
      </footer>
    </div>
  );
